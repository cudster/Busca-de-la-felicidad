...
- **Sección 4 (GitHub Actions 24/7): ✅ lista.** `.github/workflows/publish.yml` corre `python publish.py --sheet` (cron 13:10 y 22:10 UTC + manual), instala `gspread`+`google-auth`, y lee credenciales del Secret `GOOGLE_CREDENTIALS`. Los 4 Secrets cargados: `GOOGLE_CREDENTIALS`, `SHEET_ID`, `META_PAGE_TOKEN`, `IG_USER_ID`. Corrida manual verificada (success): la nube se conecta a la hoja, lee las filas aprobadas y publica las vencidas — sin depender del PC ni de Claude Code. Repo: `cudster/Busca-de-la-felicidad` (público, requerido por jsDelivr).

### Pipeline de media — Pexels (`media.py`)

Media automática con **fotos y videos reales de aviación desde Pexels** (gratis, licencia de uso comercial). Se descartó la IA: el free tier de Google no genera imágenes (requiere billing) y la IA falla en aviones específicos/diagramas; Pexels cubre todo con calidad real. `media.py --month YYYY-MM` deriva una búsqueda por post (mapea aviones: SR-71, Concorde, A350, F-35, etc., y los históricos Gimli→767/Aloha→737/Hudson→A320), busca en Pexels (foto para image, 4 fotos para carousel, video mp4 para reel), y escribe en la hoja `asset_path` (URL[s] públicas) + `preview` (miniatura `=IMAGE(...)` visible desde el celular). Overrides: `--post P05 --query "..."`. Requiere `PEXELS_API_KEY` en `.env`. Nota: Pexels/Cloudflare exige header User-Agent de navegador.

### Próximo paso

Sistema COMPLETO y probado end-to-end con media real (agosto: 20 posts con foto/video + miniaturas en la hoja; publicador ve URLs válidas). Flujo mensual: `generate_content.py` → `--to-sheet` → `media.py` → Felipe revisa miniaturas y aprueba desde el celular (escribe `feedback` para pedir otra foto; se re-corre `media.py --post X --query`) → el publicador en la nube postea solo a las 13:00/22:00 UTC. Pendiente del plan original: Módulo 5 (métricas semanales) + Módulo 4 (newsletter Beehiiv). Para que el scheduler 24/7 publique la media: subir el código nuevo (`media.py`, `sheets.py`) al repo con GitHub Desktop.
